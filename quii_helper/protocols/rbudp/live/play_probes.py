...
class _PlayProbeConfig(Protocol):
    enable_play_probes: bool


class RbUdpPlayProbeOwner(Protocol):
    CONTROL_PLAY_PROBE_INNER_DEST: int
    CONTROL_PLAY_PROBE_TAG: bytes
    CONTROL_PLAY_PROBE2_PAYLOAD: bytes
    CONTROL_PLAY_PROBE3_PAYLOAD: bytes
    CONTROL_PLAY_PROBE4_PAYLOAD: bytes
    CONTROL_PLAY_SYNC_STAGE2_LOCAL_DELTA: int
    CONTROL_PLAY_SYNC_STAGE2_REMOTE_BIAS: int
    WRAPPED_STATUS: int
    _lane_states: list[RbUdpLane]
    _peer_addr: tuple[str, int]
    config: _PlayProbeConfig
    src_ids: list[int]
...
class RbUdpPlayProbeSender:
    """Send optional diagnostic play probes during RBUDP play sync."""

    def __init__(self, owner: RbUdpPlayProbeOwner) -> None:
        self._owner = owner
    ...
    def send_play_data_probe(
        ...
        *,
        ...
        inner = build_rb_data_packet(
            ...
            dest_id=owner.CONTROL_PLAY_PROBE_INNER_DEST,
            seq=seq,
        )
        ...
        packet = build_rb_udp_wrapped_packet(
            ...
            nonce=rand16,
            tag8=owner.CONTROL_PLAY_PROBE_TAG,
        )
        owner._dbg(
            ...
            remote_id=remote_id,
            seq=seq,
            inner_dest_id=hex(owner.CONTROL_PLAY_PROBE_INNER_DEST),
    ...
    def maybe_send_play_probe(
        ...
        owner = self._owner
        if not bool(getattr(owner.config, "enable_play_probes", False)):
            return
        ...
        if probe_stage == 0 and sent_count >= 18:
            self.refresh_all_late_lanes_for_probe(lane)
            self.send_play_data_probe(
                lane,
                seq=2,
                local_id=(local_base - 0x88) & 0xFFFFFFFF,
                remote_id=(remote_id + 0x118) & 0xFFFFFFFF,
                payload=owner.CONTROL_PLAY_PROBE2_PAYLOAD,
            )
        ...
        if probe_stage == 1 and sent_count >= 35:
            self.send_play_data_probe(
                ...
                payload=owner.CONTROL_PLAY_PROBE3_PAYLOAD,
            )
            lane["play_sync_local_id"] = (
                local_base + owner.CONTROL_PLAY_SYNC_STAGE2_LOCAL_DELTA
            ) & 0xFFFFFFFF
            lane["play_sync_remote_bias"] = (
                owner.CONTROL_PLAY_SYNC_STAGE2_REMOTE_BIAS
            )
        ...
        if probe_stage == 2 and sent_count >= 48:
            self.send_play_data_probe(
                ...
                payload=owner.CONTROL_PLAY_PROBE4_PAYLOAD,
            )
