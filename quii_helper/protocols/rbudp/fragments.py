...
from quii_helper.protocols.rbudp.fragment_ack import (
    RbUdpFragmentAckCoordinator,
)
from quii_helper.protocols.rbudp.fragment_packet import RbUdpFragmentPacket
from quii_helper.protocols.rbudp.fragment_partial import (
    RbUdpFragmentPartialEmitter,
)
from quii_helper.protocols.rbudp.fragment_remainder import (
    RbUdpFragmentRemainderCoordinator,
)
from quii_helper.protocols.rbudp.fragment_stream import (
...
class RbUdpWrappedFragmentAssembler:
    ...
    def start(self, data: bytes) -> bool:
        ...
        if (
            ...
            and existing.inner_total_length == packet.inner_total_length
            and existing.have >= len(packet.payload)
        ):
            self.debug(
    ...
    def append(self, data: bytes) -> bool:
        ...
        if packet.local_id != stream.next_local_id:
            if packet.local_id < stream.next_local_id:
