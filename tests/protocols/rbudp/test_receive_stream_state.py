...
class RbUdpReceiveStreamStateTests:
    def test_advance_local_id_wraps_at_uint32_boundary(self) -> None:
        assert 2 == advance_local_id(0xFFFFFFFE, 4)

    def test_receive_ack_status_word_advertises_native_window(self) -> None:
        assert 0xFFFF0900 == receive_ack_status_word(0)
        assert 0xFFFF0900 == receive_ack_status_word(1)
        assert 0xFFFE0900 == receive_ack_status_word(2)
        assert 0x00000900 == receive_ack_status_word(0x10000)
        assert 0x00000900 == receive_ack_status_word(0x20000)
    ...
    def test_receive_stream_summary_reports_only_pending_streams(self) -> None:
        ...
        assert 2 == summary["initialized"]
        assert 3 == summary["acked"]
    ...
    def test_has_pending_receive_streams_checks_buffer_and_cache(self) -> None:
        ...
        assert not has_pending_receive_streams({(1, 2): empty})
        assert has_pending_receive_streams({(3, 4): buffered})
        assert has_pending_receive_streams({(5, 6): cached})
    ...
    def test_append_receive_stream_payload_updates_buffer_and_sequence(
        ...
        assert b"abc" == bytes(state.buffer)
        assert 1 == state.next_local_id
        assert 0xFFFFFFFE == event.local_id
        assert 3 == event.payload_len
        assert 3 == event.buffered_len
        assert 1 == event.next_local_id
        assert not event.from_cache
    ...
    def test_drain_receive_stream_cache_appends_contiguous_payloads(
        ...
        assert b"abcd" == bytes(state.buffer)
        assert 13 == state.next_local_id
        assert {20: b"z"} == state.cache
        assert [10, 12] == [event.local_id for event in events]
        assert [2, 1] == [event.payload_len for event in events]
