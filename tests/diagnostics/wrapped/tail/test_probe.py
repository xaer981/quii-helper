...
class WrappedTailProbeTests:
    def test_empty_tail_analysis_shape_is_stable(self) -> None:
        assert empty_decrypted_tail_analysis() == analyze_decrypted_tail(b"")
    ...
    def test_container_probe_without_marker_shape_is_stable(self) -> None:
        assert container_probe_without_marker() == (
    ...
    def test_container_probe_reports_false_h264_marker_context(self) -> None:
        ...
        assert marker_offset == probe["first_marker_offset"]
        assert CONTAINER_PROBE_MARKER.hex() == probe["pre_marker_tail_hex"]
        assert probe["looks_like_false_h264_container"]
        assert is_container_probe_before_start_code(blob, marker_offset)

    def test_dwords_from_bytes_ignores_trailing_partial_word(self) -> None:
        assert [0x04030201] == (
            dwords_from_bytes(b"\x01\x02\x03\x04x", "little")
        )
        assert [0x01020304] == (dwords_from_bytes(b"\x01\x02\x03\x04x", "big"))
